...
originalImage = imread('data3/00000054.jpg');
GrayImage = rgb2gray(originalImage);


% Display the image to be processed (current frame).
subplot(3, 3, 1);
...
drawnow;
caption = sprintf('Original image (current frame)');
...
%---------------------------------------------------------------------------
%----------------threshold 'originalImage' image into binaryImage!----------
%---------------------------------------------------------------------------
...
img = imread('data1/00000001.jpg');
[r,c,nothing] = size(originalImage);
bkRed = zeros(r,c,14); 
bkGreen = zeros(r,c,14); 
bkBlue = zeros(r,c,14); 
i = 1;
for j = 50:10:191
    %load one images one by one
    img = loadimage(3, j, 0);
...
subplot(3, 3, 2);
imshow(bkgrImg);  % Show the image.
title('background created', 'FontSize', captionFontSize); 

%subtract background grom image
binaryImage = backgrSub(originalImage, bkgrImg);
...
subplot(3, 3, 3);
imshow(binaryImage); 
title('Binary Image, obtained by doing background subtraction', 'FontSize', captionFontSize); 
...
for k = 1 : numberOfBlobs           % Loop through all keeper blobs.
	...
	itsarobot = allBlobAreas(k) > 50; % Dimes are small.
	if itsarobot
		plot(centroidsX(k), centroidsY(k), 'bx', 'MarkerSize', 5, 'LineWidth', 1);
	end
end
...
maskedImageDime = GrayImage; % Simply a copy at first.
...
maskedImageNickel = GrayImage; % Simply a copy at first.
...
if strcmpi(reply, 'Yes')
	...
	for k = 1 : numberOfBlobs           % Loop through all blobs.
		...
		subImage = imcrop(GrayImage, thisBlobsBoundingBox);
